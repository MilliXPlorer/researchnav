...
import { notifications, roleConfigs } from "./data";
  ...
  return (
    <div className="dashboard-shell">
      ...
      {drawerOpen && (
        <NotificationsDrawer
          readIds={readIds}
          onMarkAll={() => setReadIds(notifications.map((item) => item.id))}
          onOpen={(item) => {
            setReadIds((current) =>
              current.includes(item.id) ? current : [...current, item.id],
            );
            setActiveNav(
              role === "researcher" ? item.targetNav : primaryNav[role],
            );
            setDrawerOpen(false);
            notify(`${item.recordId} · ${item.title}`);
          }}
...
function NotificationsDrawer({
  ...
  readIds: number[];
  onMarkAll: () => void;
  onOpen: (item: (typeof notifications)[number]) => void;
  onClose: () => void;
}) {
  ...
  return (
    <div className="drawer-backdrop" onMouseDown={onClose}>
      ...
      >
        ...
        <button className="mark-read" onClick={onMarkAll}>
          Mark all as read
        </button>
        {Object.entries(grouped).map(([group, items]) => (
          <section className="notification-group" key={group}>
            ...
            {items.map((item) => (
              ...
              >
                <span
                  className={!readIds.includes(item.id) ? "unread-dot" : ""}
                />
                <span>
                  <strong>{item.title}</strong>
                  <small>{item.detail}</small>
                  <time>
                    {item.time} · {item.recordId}
                  </time>
                </span>
              </button>
            ))}
