...
class DashboardController extends DomainController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $sections = DomainAuthorization::isActiveAdministrator($actor) ? $this->adminSections() : match ($actor->role) {
        ...
        return response()
            ->json(['data' => ['schema_version' => 1, 'role' => $actor->role, 'sections' => $sections]])
    ...
    private function adminSections(): array
    {
        return [
            ...
            $this->researchSection('pending_title_validations', ResearchDocument::query()->whereHas('titleValidations', fn (Builder $validations) => $validations->where('validation_status', 'pending'))),
            $this->researchSection('flagged_similarity', ResearchDocument::query()->whereHas('sourceSimilarityResults', fn (Builder $results) => $results->where('is_flagged', true))),
    ...
    {
        $own = fn (): Builder => ResearchDocument::query()->where('submitted_by', $actor->id);
        ...
        return [
            ...
            $this->researchSection('my_flagged_similarity', $own()->whereHas('sourceSimilarityResults', fn (Builder $results) => $results->where('is_flagged', true))),
    ...
    {
        return [
            $this->researchSection('assigned_reviews', $this->assignedDocuments($actor, 'adviser')->whereIn('submission_status', ['submitted', 'under_review', 'revision_required'])),
    ...
    private function statisticianSections(User $actor): array
    {
        return [
            ...
            $this->countSection('signoffs', MethodologyReview::query()->where('statistician_id', $actor->id)->where('review_status', 'signed_off')),
    ...
    private function coordinatorSections(): array
    {
        return [
            ...
            $this->countSection('duplicate_flags', SimilarityResult::query()->where('is_flagged', true)),
        ];
    }
    ...
    private function librarianSections(): array
    {
        return [
            ...
            $this->countSection('metadata_validation', MetadataReview::query()->where('review_status', 'pending')),
    ...
    private function academicsSections(User $actor): array
    {
        return [
            ...
            $this->countSection('recommendations', SimilarityResult::query()
