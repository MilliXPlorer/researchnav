...
| PUB-2 Institute filter, selectable sort, and full pagination | **Partial** | Institute, year, and category filters are validated server-side and applied to the complete repository result set; sort remains local. | `frontend/src/CatalogPage.tsx`; `frontend/src/api.ts`; `frontend/src/data.ts`; `backend/app/Services/PublicRepositoryService.php` | UI and API use the canonical `research_documents.institute` field. | Local sorting can cover only the records returned to the browser. | Client-side sorting is not an authorization boundary, but incomplete pagination metadata can misrepresent ordering across large holdings. | Add validated server sort parameters and preserve paginator metadata. | P2 | T11 |
