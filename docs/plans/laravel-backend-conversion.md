...
- Update only the React API/CSRF integration, Vite proxy/tooling, and required loading/error/logout/pagination/accessibility state handling needed for Laravel; preserve visuals, routes, responsiveness, and the approved interaction design.
...
| `POST /api/admin/coordinators`      | `201 {"user": User}`                              | CSRF, 30/hour limiter, active Administrator only; state-changing Coordinator provisioning writes account/audit/outbox atomically, while a same-role invited replay is the defined zero-write idempotent no-op. |
...
| `POST /api/coordinator/instructors` | `201 {"user": User}`                              | CSRF, 60/hour limiter, active Coordinator only; state-changing Instructor provisioning writes account/audit/outbox atomically, while a same-role invited replay is the defined zero-write idempotent no-op.    |
...
2. Use ordered Laravel migration files with complete `up()` and `down()` methods. Add `auth_version`, Laravel `sessions`, database `cache`/`cache_locks`, `jobs`, `job_batches`, `failed_jobs`, `audit_events`, and `notification_outbox` structures and their required constraints/indexes. The database cache and locks are required for shared rate-limit counters and unique-job locks. Do not drop `app_users` or `user_sessions` in a normal migration.
...
3. Every state-changing successful provisioning request commits its account mutation, one audit event, and one `notification_outbox` insertion in the same database transaction. A request for an existing invited account already in the requested target role is instead an idempotent `201` no-op: return the unchanged user and perform no writes, audit event, outbox insertion, job enqueue, or email. A notification is never sent inline from an HTTP request.
...
2. Before any forward migration, run the legacy-phase conversion preflight (`php artisan researchnav:cutover-preflight --phase=legacy --env=<environment>`) against the current `app_users` shape and data; stop on drift, duplicate normalized emails, invalid role/status/admin combinations, orphaned inviter IDs, or missing extensions/permissions.
3. Only after the legacy phase passes, apply forward Laravel migrations while Express remains available, then deploy Laravel web and database queue/scheduler processes without switching traffic.
4. After migrations and candidate deployment, but before Express-session invalidation or any traffic switch, run the readiness-phase preflight (`php artisan researchnav:cutover-preflight --phase=readiness --env=<environment>`). Stop unless migration status, health, queue/worker, GIS and mail configuration, same-origin cookie/HTTPS behavior, and representative read-only smoke checks pass.
...
- [ ] The existing React pages preserve their visuals, routes, and responsive behavior; only API, CSRF, proxy/tooling, and required loading/error/logout/pagination/accessibility state handling and related tests changed.
...
- [ ] Every state-changing successful provisioning mutation commits its account change, one audit, and one outbox row atomically; a same-role invited idempotent `201` no-op performs no writes, audit, outbox, job, or email; notification delivery occurs only through the database queue and survives worker/process restart.
...
P0 parent plan approved
  |\
  | +--> D1 API/Data contract (`docs/contracts/**`) --------+
  +----> D2 UI/UX interaction contract (`docs/design/**`) --+
                                                           |
                                                           v (both design PRs merged)
...
D1 and D2 are parallel pre-implementation design workstreams. Both design branches must pass their child-PR gates and merge into the parent plan branch before the T1 branch is created. T5 and T6 are the only implementation tasks intended to run in parallel. All other arrows are hard dependencies. A child PR is merged into the parent plan branch before branches for its dependents are created. T7 and later start from the combined plan head containing all prerequisite child PRs.
...
| Task | Single owner                                      | Head branch                              | Isolated worktree                                                                                  | Child PR base                     |
| ---- | ------------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------- |
| D1   | API/Data Contract Agent (`app-api-data-contract`) | `design/laravel-api-data-contract`       | `C:\Users\MURALLONMILLICENTJOH\AppData\Local\Temp\opencode\researchnav-laravel-ws00-api-contract`  | `plan/laravel-backend-conversion` |
| D2   | UI/UX Designer (`app-ui-ux-designer`)             | `design/laravel-ui-ux-contract`          | `C:\Users\MURALLONMILLICENTJOH\AppData\Local\Temp\opencode\researchnav-laravel-ws00-ui-design`     | `plan/laravel-backend-conversion` |
...
plan/laravel-backend-conversion
  ^-- design/laravel-api-data-contract ---+ merged before T1
  ^-- design/laravel-ui-ux-contract ------+
...
| T6        | `package.json`; `package-lock.json`; `vite.config.ts`; `src/api.ts`; `src/api.test.ts`; `src/App.tsx`; `src/Dashboard.tsx`; `src/GoogleSignInDialog.tsx`; `src/RoleWorkspaces.tsx`; `src/types.ts`; `src/app.test.tsx`; `src/components.test.tsx`; `server/**` (deletion); `tsconfig.server.json` (deletion)                                                                                                                  |
...
Protected paths that no T1–T9 implementation workstream may modify: `algorithm/**`, `researchnav-design.md`, this plan, the merged `docs/contracts/**` and `docs/design/**` artifacts, and every React source, test, CSS, or style path not explicitly assigned to T6. T6 may make only the required loading/error/logout/pagination/accessibility state changes and related tests in its listed React files; it may not change CSS, visual design, routes, or responsive behavior. T6 must prove that deleting `server/**` is the only product-source deletion and that the complete React test suite still passes.

## Ordered workstreams

### D1 — API/data contract publication

**Entry gate:** Parent Plan PR is approved for contract design; the branch is based on its current clean head; D1 owns only `docs/contracts/**`. May run in parallel with D2.

**Tasks:**

1. Publish the approved six-path API, exact user/error/pagination, GIS/session/CSRF, authority, migration/adoption, database `cache`/`cache_locks`, audit/outbox/queue, and cutover data contracts under `docs/contracts/**` without changing product scope.
2. Reconcile names, statuses, limits, schemas, and invariants against this plan, including the database-cache dependency for shared rate limits and unique-job locks; record no unresolved implementation choice.

**Acceptance:** The contract artifacts are internally consistent with this plan, give every T1–T8 boundary a testable source of truth, introduce no seventh API path or alternate data model, and contain no implementation or design-file edits.

**Task verification:**

```bash
npx prettier --check "docs/contracts/**/*.md"
git diff --check
git diff --name-only
```

**Exit gate:** Pre-Commit Tester reports PASS on content, whitespace, links, scope, and ownership; Git Steward explicitly stages only `docs/contracts/**`; API/Data and Security reviews have no blocking finding; the D1 child PR merges into the parent plan branch before T1 is created.

### D2 — UI/UX interaction contract publication

**Entry gate:** Parent Plan PR is approved for contract design; the branch is based on its current clean head; D2 owns only `docs/design/**`. May run in parallel with D1.

**Tasks:**

1. Publish the approved interaction contract for existing sign-in, session reload, logout, provisioning, and pagination flows, including required loading/error/logout/pagination and accessibility states.
2. Map those states to the existing routes and T6-owned React files while explicitly freezing visuals, route structure, responsive behavior, CSS, and all other component behavior.

**Acceptance:** T6 has an unambiguous, accessibility-testable state contract; no redesign, route/workflow change, CSS specification, product-code edit, or API divergence is introduced.
...
**Exit gate:** Pre-Commit Tester reports PASS on content, whitespace, links, scope, and ownership; Git Steward explicitly stages only `docs/design/**`; UI/UX review has no blocking finding; the D2 child PR merges into the parent plan branch before T1 is created.

### T1 — Laravel platform and API contract foundation

**Entry gate:** Parent Plan PR is approved for execution; D1 and D2 design child PRs are merged; branch is based on the resulting clean parent-plan head; PHP 8.5, Composer, Node, and a dedicated PostgreSQL test database are available; T1 owns only its listed paths.
...
2. Configure PostgreSQL-only database, database sessions/cache/queue, mail, Google client ID, trusted proxy/HTTPS behavior, secure cookies, and fail-fast production configuration.
...
2. Add reversible conversion migrations for `auth_version`, Laravel sessions, database `cache`/`cache_locks`, database queue tables, immutable audit events, and notification outbox. The shared rate limiters and unique notification jobs must use this database-backed cache/lock store. Preserve legacy `app_users` and `user_sessions` and all existing constraints/data.
...
5. Add legacy-shape migration adoption/up-down-up tests, model mapping tests, database cache/lock tests, immutable-audit tests, and true concurrent advisory-lock tests against PostgreSQL.

**Acceptance:** Representative legacy rows survive up/down/up byte-for-byte for preserved fields; rollback removes only conversion structures; PostgreSQL-backed cache entries and cache locks are shared across application processes for rate limits and unique jobs; concurrent advisory-lock tests serialize same identity and allow unrelated identities; audit mutation is rejected; outbox idempotency constraints work; test database safeguards prevent destructive commands against a non-test database.
...
php artisan test --filter=DatabaseCacheLockTest
...
2. Implement Administrator-to-Coordinator and Coordinator-to-Instructor authority exactly, with no generic admin override. Preserve current safe rules for blocked/invited Researcher or same-target-role records and reject active/admin/incompatible records. A request for an existing invited account already in the requested target role returns the unchanged user with `201` as an idempotent no-op and performs no writes, audit, outbox, job, or email.
3. Serialize provisioning by normalized-email advisory lock. For every state-changing successful request, in one transaction update/insert the user, increment `auth_version` when authority/status changes, append exactly one audit event, and append exactly one idempotent invitation outbox row. Do not enter this mutation path for the same-role invited idempotent no-op.
...
5. Test every role/status/`is_admin` matrix case, deterministic pagination, duplicate submissions, the same-role invited idempotent `201` no-op and its zero-write/zero-delivery guarantees, races with sign-in, rollback on audit/outbox failure, and bootstrap repeat/concurrency behavior.

**Acceptance:** Exact authority matrix passes; Administrator cannot use Instructor endpoints; inactive users cannot provision; user shape is unchanged; pagination is bounded/stable; every state-changing successful provisioning mutation has exactly one audit and outbox event; the same-role invited idempotent `201` no-op returns the unchanged user and has no writes, audit, outbox, job, or email; failed transactions leave none; bootstrap is safe and idempotent.
...
4. In only the listed React files, implement the approved interaction contract's required loading, error, logout, pagination, and accessibility states. Preserve existing visuals, routes, responsive behavior, CSS, and all unrelated component behavior.
5. Add focused API-client and related component tests for CSRF bootstrap, mutation headers, single retry, stable error handling, loading/error/logout states, bounded pagination, accessibility, and unchanged user parsing. Run the complete React test suite and build.

**Acceptance:** The current UI signs in, reloads sessions, logs out, provisions, and paginates through the same routes and API paths; unsafe calls contain CSRF; retries are bounded; required loading/error/logout/pagination states are accessible and match the approved D2 contract; Node no longer starts an API server or installs Express-only packages; visuals, routes, CSS, and responsive behavior remain unchanged.
...
**Exit gate:** Frontend tests/build pass; diff confirms React component/test changes are confined to T6-owned paths and required interaction states, with no CSS/design/route/responsiveness or algorithm changes; Pre-Commit Tester and Code Reviewer approve; child PR merges after T5 or independently if T5 is still in review.
...
3. Run Composer validation/audit, Pint, PHPStan, PHPUnit, npm formatting/lint/tests/build/audit, Python tests, migration adoption/reversal, dependency audits, and repository secret scanning. T7 owns repository-wide formatting verification and remediation closure: run the full formatting checks, route any required correction to the task that exclusively owns the affected path, and rerun the repository-wide checks after those correction PRs merge; T7 must not edit another task's files. Pin action major/immutable versions according to repository policy and grant minimal workflow permissions.
4. Configure required jobs so no test silently falls back to SQLite or skips concurrency/queue tests.

**Acceptance:** A clean CI run proves every frozen contract against PostgreSQL; repository-wide formatting checks pass after any corrections are made by the existing exclusive path owners; workflow permissions are read-only unless a documented job needs more; no credentials are printed; all tools are lockfile-driven; security/architecture regressions fail CI.
...
1. Add an operator-only `researchnav:cutover-preflight` command with required, explicit `--phase=legacy` and `--phase=readiness` modes, plus a confirmation-gated Express-session invalidation command and non-secret cutover helper scripts. The legacy phase validates the unmigrated legacy schema/data and prerequisites; the readiness phase validates the migrated, deployed candidate and its operational dependencies. Both preflight modes are read-only, have no implicit default phase, and refuse unsafe environment/database targets. Other commands are idempotent where possible.
...
3. Rehearse the full sequence against disposable production-like infrastructure/data in this fixed order: restore the legacy fixture and run `researchnav:cutover-preflight --phase=legacy`; apply forward migrations; deploy/start the exact Laravel web, worker, and scheduler candidate; run `researchnav:cutover-preflight --phase=readiness`; only then invalidate Express sessions and switch traffic; then smoke, monitor, and rehearse rollback. Record row counts/checksums, both phase outputs, deployment revision and commands, session deletion count, queue/outbox state, timing, and exact commands in the PR evidence.
...
**Acceptance:** The legacy phase catches every defined pre-migration drift condition and passes before forward migrations; the readiness phase passes only after migrations and candidate deployment and before session invalidation/traffic switch; invalidation requires confirmation and leaves zero Express sessions; end-to-end flow passes under same-origin cookies and PostgreSQL; application rollback succeeds with mandatory reauthentication; no prototype data or algorithm integration appears.

**Task verification:**

Against the restored, unmigrated legacy fixture, run these commands in order:

```bash
php artisan researchnav:cutover-preflight --phase=legacy --env=testing
php artisan migrate --force --env=testing
```

Next, deploy and start the exact candidate web, worker, and scheduler processes with the environment-specific deployment command recorded in T8 evidence. After that deployment succeeds, and before any session invalidation or traffic switch, run:

```bash
php artisan researchnav:cutover-preflight --phase=readiness --env=testing
...
These are ordered gates, not interchangeable commands. Session invalidation and traffic switching are prohibited until readiness passes. Destructive rehearsal runs only on a disposable, backed-up test database after its guard is independently verified.
...
3. Write command-by-command preflight, backup, expand, deploy, session invalidation, traffic switch, smoke, monitoring, rollback, and failed-notification recovery runbooks. The cutover runbook must place `researchnav:cutover-preflight --phase=legacy` before forward migrations and `researchnav:cutover-preflight --phase=readiness` after migrations/deployment but before session invalidation/traffic switch. State clearly that rollback does not restore Express sessions.
...
Every D1–D2 and T1–T9 task follows this loop without exception:
...
6. PR Tester tests the exact committed SHA. Code Reviewer reviews every child; Security Reviewer is mandatory for D1, T1–T5, T7, T8, and any correction touching auth/data/config/CI.
...
| --------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| S0 Contract/threat gate     | Before T1                      | Merged D1/D2 artifacts, six-path inventory, trust boundaries, stable errors, authority matrix, data classification, same-origin and interaction-state decisions | Either design PR unmerged or any ambiguous auth/authority/data/interaction contract                     |
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Existing `app_users` schema/data drift        | Run the explicit legacy-phase preflight before forward migration to verify exact columns/constraints/roles/statuses/lowercase uniqueness and representative backup restore. Stop before migration on any mismatch; do not auto-coerce production data. |
...
| Frontend regression                           | T6 path restriction, approved D2 state contract, complete React suite/build, six-path contract tests, and same-origin browser smoke. No CSS/design/route/responsiveness changes and no cutover on UI/auth smoke failure.                               |
...
- [ ] D1 API/data and D2 UI/UX design branches have passed review and merged into the parent plan branch; T1 has not been created from an earlier head.

### Final verification gate

- [ ] D1–D2 and all T1–T9 child PRs are merged and linked, with no unmerged correction branch.
...
- [ ] The legacy-phase preflight passed before forward migrations, and the readiness-phase preflight passed after migrations/deployment and before session invalidation/traffic switch; Laravel web, worker, scheduler, HTTPS/same-origin, GIS, mail, and monitoring checks pass.
...
- [ ] D1 — API/data contract merged before T1 (PR: ___; SHA: ___; evidence: ___)
- [ ] D2 — UI/UX interaction contract merged before T1 (PR: ___; SHA: ___; evidence: ___)
...
- [ ] T6 — React CSRF/proxy/approved states and Express removal merged (PR: ___; SHA: ___; evidence: ___)
...
- [ ] PostgreSQL database cache/cache-lock sharing test
...
- [ ] `researchnav:cutover-preflight --phase=legacy --env=testing` before forward migrations
- [ ] `researchnav:cutover-preflight --phase=readiness --env=testing` after migrations/deployment and before session invalidation/traffic switch
...
- React CSRF/proxy/tooling and required loading/error/logout/pagination/accessibility state updates without visual/route/responsive changes, comprehensive PostgreSQL CI/security tests, and setup/cutover/rollback docs.
...
- [ ] Every state-changing successful provisioning mutation has one atomic audit/outbox pair; same-role invited idempotent `201` no-ops have no writes, audit, outbox, job, or email; database queue notification survives retries/restarts.
- [ ] Bootstrap, legacy/readiness preflight, cutover, and rollback commands and docs are validated in the required order.
...
| ------------------------------ | ----------------------- | ------- | ---------------------------------------- | ------------ |
| D1 API/data contract           | API/Data Contract Agent | Pending | `design/laravel-api-data-contract`       |              |
| D2 UI/UX interaction contract  | UI/UX Designer          | Pending | `design/laravel-ui-ux-contract`          |              |
...
| T6 React state/Express cutover | Frontend Builder        | Pending | `feat/laravel-frontend-cutover`          |              |
...
- Merge the D1 API/data and D2 UI/UX design PRs before creating T1; freeze their artifacts for implementation.
...
- [ ] D1 contract merged before T1: ___
- [ ] D2 design merged before T1: ___
- [ ] T1 merged: ___
- [ ] T2 merged: ___
- [ ] T3 merged: ___
- [ ] T4 merged: ___
